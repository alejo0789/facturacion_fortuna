...
def generate():
    ...
    docs_to_export = [1700, 1737, 1699, 1736]
    output_lines = []
    
    output_lines.append("-- ==========================================================")
    output_lines.append("-- SCRIPT LOCAL DE INICIALIZACION (ORACLE DOCKER)            ")
    output_lines.append("-- ==========================================================\n")
    
    # User prep
    # In Docker Oracle XE, setting up user MANAGER
    ...
    tables = ['MNGDOC', 'MNGMCN', 'MNGCTA', 'MNGTDC']
    for t in tables:
        ...
        output_lines.append(f"-- Estructura de {t}")
    ...
    for row in cursor.fetchall():
        vals = [format_value(v) for v in row]
        output_lines.append(f"INSERT INTO MANAGER.MNGTDC ({','.join(cols)}) VALUES ({','.join(vals)});")

    output_lines.append("\n-- Vista Aprobados")
    output_lines.append("""CREATE OR REPLACE VIEW MANAGER.VW_TESO_APROBADO_CXP_CXC AS
SELECT
